...
export default class Services extends Component {
	...
	submitHandler = async e => {
		...
		var l = this.state.selectedItems.length,
			selectedItems = []
		for (let i = 0; i < l; i++) selectedItems[i] = [...this.state.selectedItems[i]]
		...
		const ServiceItems = props => {
				...
				return (
					...
					>
						...
						<div className={'div__services__items ' + iS.color}>{allSectionItems}</div>
					</div>
				)
			},
			SelectedServiceItems = props => {
				let s = props.section,
					...
					allSelectedServiceItems = s.map((item, j) => (
						<span className={'services__item--' + sS.slug} key={j} onClick={e => this.removeItem(e, i, j, item)}>
							{item}
						</span>
					))
			...
			services = this.state.items.map((s, i) => <ServiceItems section={s} index={i} key={i} />),
			selectedServices = this.state.selectedItems.map((s, i, arr) => (
				<SelectedServiceItems section={s} index={i} selectedItems={arr} key={i} />
			)),
