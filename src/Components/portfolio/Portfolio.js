...
export default class Portfolio extends Component {
  ...
  render() {
    ...
    const links = portfolioData.portfolioPieces.map((pc, i) =>
        <div className={"portfolio__li__wrapper" + (this.state.pieceIndex !== null ? (i > (this.state.pieceIndex + (2 - (this.state.pieceIndex % 3))) ? " hidden" : "" ) : "")} onClick={()=>this.state.pieceIndex!==i ? this.pieceIndex(i) : null} key={i}>
          <Link to={"/portfolio/" + (this.state.pieceIndex===i ? "" : (pc.slug + "/"))}>
            <li className={"portfolio__li" + (this.state.pieceIndex === i ? "--selected" : "")} style={pc.style}>
            </li>
            <span className={"portfolio__li__title" + (this.state.pieceIndex === i ? "--selected" : "")}>
              {(()=>pc.name.split("").map(l=>l===l.toUpperCase() ? (" " + l) : l).join(""))()}
            </span>
          </Link>
        </div>
      )
      ...
      if (this.state.portfolioPiece) {
        portfolioPiece = <PortfolioPiece
          numSlides={portfolioData.portfolioPieces[this.state.pieceIndex].numSlides}
