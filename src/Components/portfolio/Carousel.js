...
export default class Carousel extends Component {
	...
	arrowHandler = side => {
		if (side === 'left') {
			this.state.carouselOffset < 0
				? this.setState({
						carouselOffset: this.state.carouselOffset + this.state.slideWidth
				  })
				: this.setState({
						carouselOffset: (this.state.numSlides - 1) * -this.state.slideWidth
				  })
		} else if (side === 'right') {
	...
	render() {
		...
		const slideDots = this.state.slideDots.map((dot, i) => {
			return (
				<div
					onClick={_ => this.setState({ carouselOffset: i * this.state.slideWidth * -1 })}
					className={
						'carousel__dot' + ((this.state.carouselOffset / this.state.slideWidth) * -1 === i ? ' current' : '')
					}
					key={`dot${i}`}
				/>
			)
		...
		return (
			<div className="carousel__wrap">
				<div className="carousel__arrow--left" onClick={_ => this.arrowHandler('left')}>
					<span>&lsaquo;</span>
				</div>
				<div className="carousel__arrow--right" onClick={_ => this.arrowHandler('right')}>
					<span>&rsaquo;</span>
				</div>

				<div className="carousel__dots__wrap">
					<div className="carousel__dots">{slideDots}</div>
				</div>
