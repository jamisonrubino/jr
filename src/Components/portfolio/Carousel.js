...
export default class Carousel extends Component {
  ...
  arrowHandler(side) {
    if (side === "left") {
      ...
      this.state.carouselOffset >
      (this.state.numSlides - 1) * -this.state.slideWidth
        ? this.setState({
            carouselOffset: this.state.carouselOffset - this.state.slideWidth
          })
  ...
  render() {
    ...
    const slideDots = this.state.slideDots.map((dot, i) => {
      return (
        <div
          onClick={() =>
            this.setState({ carouselOffset: i * this.state.slideWidth * -1 })
          }
          className={
            "carousel__dot" +
            ((this.state.carouselOffset / this.state.slideWidth) * -1 === i
              ? " current"
              : "")
          }
          key={`dot${i}`}
    ...
    return (
      <div className="carousel__wrap">
        ...
        <div
          ...
          style={carouselStyle}
          onClick={() =>
            this.state.data.url ? window.open(this.state.data.url) : null
          }
        >
          {carousel}
        </div>
